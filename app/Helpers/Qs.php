...
class Qs
{
 ...
 <div class="alert alert-success alert-bordered">
    ...
    public static function getTeamTAS()
    {
        return ['admin', 'super_admin', 'accountant', 'student'];
    }
    ...
    {
        $data = ['name', 'email', 'phone', 'phone2', 'dob', 'gender', 'address', 'bg_id', 'nal_id', 'state_id', 'lga_id','status'];
    ...
    {
        $data = ['my_class_id', 'section_id', 'my_parent_id', 'dorm_id', 'dorm_room_no', 'year_admitted', 'house', 'age', 'previous_school', 'adm_no', 'status', 'year','college_id','level','major','minor','department_id'];
    ...
    public static function userIsTeamTAS()
    {
        return Auth::user()->hasAnyRole(self::getTeamTAS());
    }
    ...
    public static function usercollege()
    {
      $user = Auth::user()->id;
    ...
    public static function userIsMyChild($student_id)
    {
    ...
    {
        return self::json(__('msg.clean_ok'));
    }
    ...
    // public static function permission($user_id,$permission)
    // {
    //     return UserPermission::where('user_id', $user_id)->where('permission', $permission)->first();
    // }


    // public static function hasPermission($permission)
    // {
    //     return in_array(self::permission(Auth::user()->id,$permission));
    // }


    public static function student_fees($student, $year, $exam)
    {
        ...
        // If there are no payment records at all
        if ($records->isEmpty()) {
            return [
                'status' => -1, // -1 means no payment record at all
                'usd_paid' => 0,
                'lrd_paid' => 0,
                'record' => [],
        ...
        // Check for records where amount paid is less than expected
        ...
        // Split into LRD and USD and sum up payments
        ...
        return [
            'status' => $notFullyPaid->isNotEmpty() ? 1 : 0, // 1 = not fully paid, 0 = fully paid
    ...
    public static function published_grades($exam_id)
    {
        $exam = Exam::find($exam_id);
        return $exam ? $exam->published : false;
    }
    
        public static function department_code($student_id)
    {
        $student = StudentRecord::where('user_id',$student_id)->first();
        if(!$student->department_id)
        {
        $department = MyClass::find($student->my_class_id);
        return $department->class_type->code;
        }
        else{
        $department = ClassType::find($student->department_id);
