...
class MarkRepo
{
    ...
    public function getSubTotalTerm($st_id, $sub_id, $term, $year)
    {
        // You may wish to get exam id from term Exam::where(['term' => $term, 'year' => $yr])
        $d = ['student_id' => $st_id, 'subject_id' => $sub_id, 'year' => $year];
    ...
    public function getExamTotalTerm($exam, $st_id, $year)
    {
        $d = ['student_id' => $st_id, 'exam_id' => $exam->id,'year' => $year];

        $tex = 'tex'.$exam->term;
        $mk =DB::table('marks')->where('student_id', $st_id)->where('year', $year)->where('exam_id', $exam->id)->where($tex, '>', 0)->sum('grade_get');
        return $mk;
    ...
    public function getExamAvgTerm($exam, $st_id, $year)
    {
        $d = ['student_id' => $st_id, 'exam_id' => $exam->id,'year' => $year];
        ...
        $avg = $mk->select($tex)->avg('grade_get');
        return round($avg, 1);
    ...
    public function getSubCumTotal($tex3, $st_id, $sub_id,  $year)
    {
        $tex1 = $this->getSubTotalTerm($st_id, $sub_id, 1,  $year);
        $tex2 = $this->getSubTotalTerm($st_id, $sub_id, 2,  $year);
    ...
    public function getSubjectMark($exam,  $sub_id, $st_id, $year)
    {
        $d = [ 'exam_id' => $exam->id, 'subject_id' => $sub_id, 'student_id' => $st_id, 'year' => $year ];
        $tex = 'tex'.$exam->term;

        return Mark::where($d)->select($tex)->get()->first()->$tex;
    }

    public function getSubPos($st_id, $exam, $sub_id, $year)
    {
        $d = ['exam_id' => $exam->id,  'subject_id' => $sub_id, 'year' => $year];
        $tex = 'tex'.$exam->term;

        $sub_mk = $this->getSubjectMark($exam, $sub_id, $st_id, $year);
    ...
    // {
    //     $d = ['student_id' => $st_id, 'exam_id' => $exam->id,  'year' => $year ]; $all_mks = [];
    //     $tex = 'tex'.$exam->term;
    ...
    //         $all_mks[] = $this->getExamTotalTerm($exam, $s->student_id, $year);
    //     }
    //     rsort($all_mks);
