        ...
        <table style="width:100%; border-collapse:collapse; border: 1px solid #000; margin: 10px auto;" border="1">
            ...
            <tr>
                ...
                @foreach($subjects as $sub)
                    <th rowspan="2">{{ strtoupper($sub->subject->name) }}-{{ 'session' }}-{{ $sub->session }}</th>
                @endforeach
             {{--   @if($ex->term == 3)
            ...
            @foreach($students as $s)
                <tr>
                    ...
                    <td style="text-align: center">{{ $s->adm_no }}</td>
                    @foreach($subjects as $sub)
                        <td>{{ $marks->where('student_id', $s->user_id)->where('subject_id', $sub->id)->first()->grade->name ?? '-' ?: '-' }}</td>
                    @endforeach
