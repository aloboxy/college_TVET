...
@section('page_title', 'Manage Class Cohorts')
@section('content')

    <div class="card">
        <div class="card-header header-elements-inline">
            <h6 class="card-title">Manage Class Cohorts</h6>
            {!! Qs::getPanelOptions() !!}
        </div>

        <div class="card-body">
            <ul class="nav nav-tabs nav-tabs-highlight">
                <li class="nav-item"><a href="#new-section" class="nav-link active" data-toggle="tab">Create New Cohort</a></li>
                <li class="nav-item dropdown">
                    <a href="#" class="nav-link dropdown-toggle" data-toggle="dropdown">Manage Cohort</a>
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            <form class="ajax-store" method="post" action="{{ route('sections.store') }}">
                                ...
                                <div class="form-group row">
                                    <label class="col-lg-3 col-form-label font-weight-semibold">Class WhatsApp link</label>
                                    <div class="col-lg-9">
                                        <input name="whatsapplink" value="{{ old('whatsapplink') }}" type="text" class="form-control" placeholder="https://google.com">
                                    </div>
                                </div>
                                ...
                                <div class="form-group row">
                                    ...
                                    <div class="form-group">
                                        <select class="select form-control" id="active" name="active" required data-fouc data-placeholder="Choose..">
                                            ...
                                            <option {{ old('active'  == '0' ? 'selected' : '') }} value="0">InActive</option>
                                        </select>
                                    </div>
                                </div>

                                <div class="form-group row">
                                    <label for="active" class="col-lg-3 col-form-label font-weight-semibold">Planning</label>
                                    <div class="form-group">
                                        <select class="select form-control" id="planning" name="planning" required data-fouc data-placeholder="Choose..">
                                            <option value=""></option>
                                            <option {{ old('planning'  == '1' ? 'selected' : '') }} value="1">Avaliable</option>
                                            <option {{ old('planning'  == '0' ? 'selected' : '') }} value="0">Not Avaliable</option>
                                        </select>
                                    </div>
                                </div>
                    ...
                    <div class="tab-pane fade" id="c{{ $d->id }}">                         <table class="table datatable-button-html5-columns">
                            ...
                            @foreach($sections->where('my_class.id', $d->id) as $s)
                                <tr>
                                    ...
                                    <td>@if($s->planning == 1)
                                        {{ 'Planning on For Cohort' }}</td>
                                        @else
                                        {{ 'Planning Closed for Cohort' }}
                                        @endif
